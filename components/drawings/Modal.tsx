...
export const ModalWrapper2D = ({ userId, uploadFile, email, refetchUsers }: { uploadFile: any, userId: number, email: string, refetchUsers: any }) => {
    const [isOpen, setIsOpen] = useState(false);
    return (
        <div>
            <button
            ...
            <SpringModal2D isOpen={isOpen} userId={userId} setIsOpen={setIsOpen} email={email} refetchUsers={refetchUsers} uploadFile={uploadFile} />
        </div>
    );
};

const SpringModal2D = ({ isOpen, setIsOpen, uploadFile, userId, email, refetchUsers }: { isOpen: boolean, setIsOpen: Function, userId: number, uploadFile: any, email: string, refetchUsers: any }) => {
    return (
        <AnimatePresence>
            {isOpen && (
                <motion.div
                ...
                >
                    ...
                    >
                        ...
                        <div className="relative z-10 space-y-6">
                            ...
                            <UploadFile uploadFile={uploadFile} userId={userId} setIsOpen={setIsOpen} email={email} fileType={""} refetchUsers={refetchUsers} />
                        </div>
                    </motion.div>
                </motion.div>
            )}
